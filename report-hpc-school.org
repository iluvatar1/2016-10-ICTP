...
** Matrix distribution
   1. Initialization and distribution arbitrary large square identity
      matrix, distributed across the processes.
   2. The matrix must be printed by process 0. 
   3. Consider the case where the matrix size is not a multiple of 
